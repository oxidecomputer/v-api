...
import { vApiRequest, VApiRequestError } from './client'
...
describe('vApiRequest', () => {
  ...
  it('resolves without throwing on an empty response body', async () => {
    server.use(
      http.post(
        `${HOST}/login/oauth/github/code/token`,
  ...
  it('falls back to a generic message when the error body has no message field', async () => {
    server.use(
      http.get(`${HOST}/self`, () => HttpResponse.json({ error_code: 'boom' }, { status: 500 })),
    )

    const err = (await vApiRequest(HOST, '/self', { method: 'GET' }).catch(
      (e) => e,
    )) as VApiRequestError
